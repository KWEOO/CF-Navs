...
标准 Netscape Bookmark HTML 在浏览器内转换为共享 `ImportReq`，原始文件不上传。只导入 HTTP(S) 链接，仅忽略已知的书签栏/其他书签导出容器；普通第一层文件夹保留为一级分类，第二层映射为二级分类，更深路径使用 `›` 压平进二级标题。文件内安全图标优先保留，没有图标时按域名生成 Favicon.im 地址。导入前会根据当前后台数据预览新建/复用分类、无效链接、文件内重复和现有数据重复；同一完整分类路径下的规范化 URL 才判重，不同分类中的相同 URL 保留。Worker 使用 `dedupe_bookmarks` 做最终权威去重，避免确认到提交之间的数据变化产生重复。Worker 对覆盖和合并结果统一执行分类 ID 重映射，先写一级、再写二级、最后写书签，同时重写 `parent_id` 和 `category_id`，整个 D1 batch 不会留下半次导入状态。
